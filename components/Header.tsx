...
const LoginIcon = () => (
    <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
);

const Header: React.FC<HeaderProps> = ({ onGoHome, onGoToFAQ, onGoToAbout, onOpenTerms, onOpenPrivacy, onOpenAuthModal, guestGenerations }) => {
        ...
        <div className="max-w-6xl mx-auto py-4 px-4 sm:px-6 lg:px-8 flex flex-row items-center justify-between">
          ...
          <div className="relative flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              {loading ? (
                <div className="w-48 h-9 bg-white/20 rounded-lg animate-pulse"></div>
              ) : user ? (
                userMenuDesktop
              ) : (
                <button onClick={onOpenAuthModal} className="bg-white text-brand-secondary font-bold py-2 px-4 rounded-lg text-sm transition-all duration-300 transform hover:scale-105 shadow flex items-center justify-center">
                    Masuk / Daftar
                </button>
              )}
            </div>
                    ...
                    {loading ? <div className="p-4"><div className="h-8 bg-gray-200 rounded animate-pulse"></div></div> : (
                        user && profile ? (
                            ...
                            <div className="p-2 border-b border-gray-100">
                              <button onClick={() => { onOpenAuthModal(); setIsMobileMenuOpen(false); }} className="w-full bg-brand-primary hover:bg-brand-secondary text-white font-bold py-2 px-4 rounded-md text-sm transition-all duration-300 flex items-center justify-center">
                                <LoginIcon />
                                Masuk / Daftar
                              </button>
                            </div>
